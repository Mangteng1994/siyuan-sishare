  ...
  <div class="pages-pub-shell">
    <main class="pages-pub-main">
      <section class="pages-pub-header">
        <div class="pages-pub-title">${escapeHtml(title || "Untitled")}</div>
...
function preparePublishedContent(content: string, options: { targetDir: string; sharedBase: string }): { contentHtml: string; tocHtml: string } {
  const parser = getDomParser();
  if (!parser) {
    return {
      contentHtml: content,
